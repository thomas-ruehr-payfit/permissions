...
import { INITIAL_MANAGER_FLOW } from './types';
import type { ManagerFlowState } from './types';

const STEPS = [
  { num: 1 as const, label: 'Who are you inviting?' },
  { num: 2 as const, label: 'Reports' },
  { num: 3 as const, label: 'Permissions' },
  { num: 4 as const, label: 'Review & send' },
];

const STEP_TITLES: Record<number, string> = {
  1: 'Who is the manager?',
  2: 'Who do they manage?',
  3: 'What can they do?',
  4: 'Review & confirm',
};

function canProceed(step: number, state: ManagerFlowState): boolean {
  switch (step) {
    case 1:
      if (!state.whoType) return false;
      if (state.whoType === 'existing') return !!state.selectedEmployeeId;
      return !!state.newName.trim() && !!state.newEmail.trim();
    case 2:
      return state.manageeGroupIds.length > 0 || state.manageeEmployeeIds.length > 0;
    case 3:
...
export function AddManagerPage() {
  ...
  const { addManager } = useUsers();
  const [step, setStep] = useState<StepNum>(1);
  const [state, setState] = useState<ManagerFlowState>(INITIAL_MANAGER_FLOW);
  ...
  const handleSubmit = () => {
    ...
    addManager({
      ...
      status: 'pending',
      manageeGroupIds: state.manageeGroupIds,
      manageeEmployeeIds: state.manageeEmployeeIds,
  ...
  const stepContent = {
    1: <StepWho state={state} setState={setState} />,
    2: <StepPerimeter state={state} setState={setState} />,
    3: <StepPermissions state={state} setState={setState} />,
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            fontSize: 13, color: 'var(--text2)', padding: '4px 8px', borderRadius: 5, transition: 'color 0.1s',
          }}
      ...
      <div style={{ height: 2, background: 'var(--border)', flexShrink: 0 }}>
        <div style={{ height: '100%', width: `${progress * 100}%`, background: 'var(--mgr)', transition: 'width 0.35s ease' }} />
      </div>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', padding: '28px 32px', gap: 20 }}>
        ...
        <div style={{
          ...
          border: '0.5px solid var(--border2)', borderRadius: 12, padding: '10px 8px',
          display: 'flex', flexDirection: 'column', gap: 2, alignSelf: 'start',
        }}>
          {STEPS.map(s => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: isDone ? 'pointer' : 'default', transition: 'all 0.1s', textAlign: 'left',
                }}
              >
                <div style={{
                  width: 24, height: 24, borderRadius: '50%', flexShrink: 0,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  background: isActive ? 'var(--mgr)' : 'transparent',
                  border: `1.5px solid ${isActive ? 'var(--mgr)' : isDone ? 'var(--text3)' : 'var(--border2)'}`,
        ...
        <div style={{ flex: 1, background: 'var(--surface)', border: '0.5px solid var(--border2)', borderRadius: 12, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ flex: 1, overflow: 'auto', padding: '32px 40px' }}>
            ...
            <div style={{ marginTop: 20 }}>{stepContent}</div>
          </div>
          ...
          <div style={{ padding: '14px 32px', flexShrink: 0, borderTop: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              ...
              style={{
                ...
                background: ok ? 'var(--mgr)' : 'var(--border2)',
                color: ok ? 'white' : 'var(--text3)',
                fontSize: 13, fontWeight: 500, cursor: ok ? 'pointer' : 'default', transition: 'all 0.12s',
              }}
            >
